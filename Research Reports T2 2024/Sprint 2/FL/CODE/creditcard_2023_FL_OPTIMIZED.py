...
# Preprocessing function specific to creditcard_2023.csv
def preprocess_creditcard_dataset():
    dataset_df = pd.read_csv("creditcard_2023.csv")  # Adjust the file path if necessary

    # Use SMOTE to balance the dataset (if necessary)
    def balance_classes_with_smote(data_df):
        features = data_df.drop(columns=["Class"])
    ...
    # Drop unnecessary columns if any (you can adjust this based on dataset)
    selected_columns = [col for col in dataset_df.columns if col != "Time"]  # Assuming 'Time' is not useful
...
# Preprocess the creditcard_2023.csv dataset
